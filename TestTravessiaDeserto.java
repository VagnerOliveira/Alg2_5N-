...
* @author Vagner Rosales de Oliveira
* Trabalho 2
*/
...
public class TestTravessiaDeserto {
	...
	public void testVoltar() {
		...
		if (fuel<1) {  				// Testa o voltar
			jogo.voltar();
			assertEquals(pos, jogo.getPos());
			assertEquals(fuel, jogo.getFuel());
		}
		else if (pos==0) {			// Testa o nao voltar
			jogo.voltar();
			assertEquals(pos, jogo.getPos());
			assertEquals(fuel, jogo.getFuel());
		}
	...
	//Testa CARREGAR
		...
		public void testDescarregar() {
			...
			if (fuel>0) {			// PODE DESCARREGAR
				jogo.descarregar();
				assertEquals(fuel -1 , jogo.getFuel());
			}
			if (fuel==0) {			//NAO PODE DESCARREGAR 
				jogo.carregar();    //BUG COM O COMBUSTÍVEL NO ZERO DEVERIA ENCERRAR O JOGO.
				assertEquals(fuel -1, jogo.getFuel()); 
			}
